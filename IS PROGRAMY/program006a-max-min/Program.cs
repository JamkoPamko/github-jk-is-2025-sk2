...
while (again == "a")
{
    ...
    //Hledání maxima, minima, pozice minima
    ...
    if(max >= 3)
    {
        ...
        Console.WriteLine("========================");
        Console.WriteLine();
        Console.WriteLine($"Přesýpací hodini o velikosti {max}");

        //tento cyklus se stará o to, aby se vykraslil správný počet ...
        for (int i = 0; i < max ;i++)
        {
            int spaces, stars;

            if (i < max / 2) {
                //počet mezer v i-tém řádku
                spaces = i;
                ...
                //dolní polovina - počet mezerv i-tém řádku
                spaces = max - i - 1;
                if (max % 2 == 1)
                {
                    stars = 2 * (i - max / 2) + 1;
